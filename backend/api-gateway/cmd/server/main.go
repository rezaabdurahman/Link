...
import (
	...
	"github.com/link-app/shared-libs/database"
...
func main() {
	...
	// Setup database connection for health checks using shared-libs
	dbConfig := database.DefaultConfig()
	dbConfig.Type = database.DatabaseTypeGORM // API Gateway uses GORM for simplicity
	sharedDB, err := database.NewDatabase(dbConfig)
	if err != nil {
		loggerInstance.WithError(err).Warn("Failed to connect to database - continuing without DB health checks")
	} else {
		// Get SQL instance for health checker compatibility
		db, sqlErr := sharedDB.GetSQL()
		if sqlErr != nil {
			loggerInstance.WithError(sqlErr).Warn("Failed to get SQL instance - continuing without DB health checks")
		} else {
	...
	router.Use(errors.ErrorHandler(loggerInstance)) // Consistent error handling middleware
	...
	// AI routes (proxied to summarygen-service)
	aiGroup := authGroup.Group("/ai")
	{
		aiGroup.Any("/*path", k8sHandler.ProxyToK8sService("summarygen-svc"))
	}
	
	// Cue card routes for conversational suggestions
	cueCardsGroup := authGroup.Group("/ai/cue-cards")
	{
		// Rate limiting for AI operations - more restrictive
		cueCardsGroup.Use(middleware.NewRateLimitMiddleware(20, time.Minute, "cue_cards")) // 20 requests per minute
		
		cueCardsGroup.POST("/generate", func(c *gin.Context) {
			// Add specific logging for cue card generation
			c.Header("Content-Type", "application/json")
			k8sHandler.ProxyToK8sService("summarygen-svc")(c)
		})
		
		cueCardsGroup.POST("/generate-message", func(c *gin.Context) {
			// Add specific logging for message generation
			c.Header("Content-Type", "application/json")
			k8sHandler.ProxyToK8sService("summarygen-svc")(c)
		})
	}
	
	// Tonality analysis routes
	tonalityGroup := authGroup.Group("/ai/tonality")
	{
		tonalityGroup.Use(middleware.NewRateLimitMiddleware(10, time.Minute, "tonality")) // 10 requests per minute
		
		tonalityGroup.POST("/analyze", func(c *gin.Context) {
			c.Header("Content-Type", "application/json")
			k8sHandler.ProxyToK8sService("summarygen-svc")(c)
		})
	}
