...
func TestServiceConfigInitialization(t *testing.T) {
	...
	expectedServices := []string{
		...
		"discovery-svc", "priority-svc", "search-svc", "opportunities-svc",
	}
...
func TestRouteToServiceLoadBalancer(t *testing.T) {
	tests := []struct {
		...
		{"/conversations/priority", "priority-svc", false},
		{"/search/users", "search-svc", false},
		{"/opportunities/nearby", "opportunities-svc", false},
		{"/unknown/path", "", true},
		{"/", "", true},
	}
...
func TestGetAllServiceLoadBalancers(t *testing.T) {
	...
	assert.Len(t, services, 8) // Should have all 8 services
...
func TestGetLoadBalancerStats(t *testing.T) {
	...
	assert.Len(t, stats, 8) // Should have stats for all 8 services
